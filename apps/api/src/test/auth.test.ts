...
describe("auth endpoints", () => {
  it("registers a new user with normalized credentials and safe DTOs", async () => {
    ...
    expect(response.body.refreshToken).toMatch(/^[A-Za-z0-9-_]{64}$/);
  ...
  it("logs in with valid credentials and rejects invalid credentials", async () => {
    ...
    expect(success.body.accessToken).toMatch(
      /^[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+$/,
    );
  ...
  it("rotates refresh tokens", async () => {
    ...
    expect(refreshed.body.refreshToken).toMatch(/^[A-Za-z0-9-_]{64}$/);
...
describe("users, chats, and messages", () => {
  it("requires auth for user search and returns limited safe DTOs", async () => {
    ...
    expect(
      response.body.every(
        (user: { passwordHash?: string }) => user.passwordHash === undefined,
      ),
    ).toBe(true);
    expect(
      response.body.some((user: { id: string }) => user.id === current.user.id),
    ).toBe(false);
  });
  ...
  it("requires auth to create chats and never uses a hardcoded current user", async () => {
    ...
    const participantIds = response.body.participants.map(
  ...
  it("lists only chats where the current user is a participant", async () => {
    ...
    expect(response.body.map((chat: { id: string }) => chat.id)).toEqual([
      aliceChat.body.id,
    ]);
  });
  ...
  it("persists created messages and exposes them as chat last messages", async () => {
    ...
    expect(chats.body[0].lastMessage).toMatchObject({
      id: created.body.id,
      text: "Hello Bob",
    });
  });
